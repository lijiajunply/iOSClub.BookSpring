...
class CosSha1Pure
{
    ...
    public string GetDigest()
    {
        ...
        foreach (var t in _H)
        {
            var digestByte = BitConverter.GetBytes(t);
            foreach (var t1 in digestByte)
            {
                digest.Append(t1.ToString("x2"));
            }
        }
    ...
    {
        var sha1Hex = new StringBuilder();
        var sha1Binary = HashFinal();
        foreach (var t in sha1Binary)
        {
            sha1Hex.Append(t.ToString("x2"));
        }
    ...
    public void HashCore(byte[] rgb, int ibStart, int cbSize)
    {
        ...
        if (cbSize % BLOCK_SIZE_BYTES == 0) return;
    ...
    {
        for (var i = 16; i < 80; i += 8)
        {
            var val = buff[i - 3] ^ buff[i - 8] ^ buff[i - 14] ^ buff[i - 16];
            buff[i] = (val << 1) | (val >> 31);
